...
func addChromeHeaders(kvs []keyValues) []keyValues {
	values := map[string]string{}
	isConnect := false
	for _, kv := range kvs {
		if strings.HasPrefix(kv.key, "X-Sneak") {
			isConnect = true

		}
		values[strings.ToLower(kv.key)] = kv.values[0]
	}
	if isConnect {
		return []keyValues{
	...
	for _, kv := range kvs {
		if strings.ToLower(kv.key) == "connection" {
			newKvs[0].values = kv.values
		} else if strings.ToLower(kv.key) == "sec-ch-ua" {
